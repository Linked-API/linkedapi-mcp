...
import LinkedApi, { TNvSearchCompanyParams } from "linkedapi-node";
...
const salesNavigatorSearchCompanies = async (
  ...
  const params = nvSearchCompaniesSchema.parse(args);
  const progressToken = "nv_search_companies";
  const workflow = await linkedapi.salesNavigatorSearchCompanies(
    params as TNvSearchCompanyParams,
  );
